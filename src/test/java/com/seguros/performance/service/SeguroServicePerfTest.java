...
public class SeguroServicePerfTest {
    ...
    // private final static JUnitPerfReportingConfig PERF_CONFIG =
    ...
    // mockSeguros.add(new Seguro("Seguro Básico", "Seguro descripcion",
    // TipoSeguro.VIDA, 1000.0));
    // when(segurorepo.findAll()).thenReturn(mockSeguros);

    // // Mock de un método que busca un seguro por nombre
    // when(segurorepo.findByNombre("Seguro
    // Básico")).thenReturn(mockSeguros.get(0));

    // // Mock de un método para eliminar un seguro por ID
    // doNothing().when(segurorepo).deleteById(1L);

    // nombreUnico = "Seguro inicial " + System.currentTimeMillis() + "_" +
    // java.util.UUID.randomUUID().toString();
    // Seguro seguro = new Seguro(nombreUnico, "Cobertura estándar",
    // TipoSeguro.VIDA, 1000.0);

    // when(segurorepo.save(any(Seguro.class))).thenReturn(seguro);

    // seguro = segurorepo.save(seguro); // Guardamos el seguro y obtenemos el ID
    // generado
    ...
    // + java.util.UUID.randomUUID().toString();
    // seguroService.crearSeguro(nombreUnicoTest, "Cobertura básica", "VIDA",
    // 1200.0);
    // }
    ...
    // boolean result = seguroService.editarSeguro(seguroId, nombreUnico, "Cobertura
    // extendida", "VIDA", 1500.0);
    ...
    // @Test
    // @JUnitPerfTest(threads = 10, durationMs = 1000, rampUpPeriodMs = 500)
    // @JUnitPerfTestRequirement(meanLatency = 100, allowedErrorPercentage = (float)
    // 0.05)
    ...
    // @JUnitPerfTestRequirement(meanLatency = 700, allowedErrorPercentage = (float)
    // 0.05)
